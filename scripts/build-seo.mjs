...
const nav = (active) => `
  <header class="header">
    <div class="container header-inner">
      <a class="logo" href="/"><span class="logo-badge"><svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><rect x="4" y="4" width="40" height="40" rx="11" fill="#F7B33D"/><g transform="translate(24 24)"><circle cx="-7" cy="-5" r="1.2" fill="#161616"/><circle cx="0" cy="-7" r="1.2" fill="#161616"/><circle cx="7" cy="-5" r="1.2" fill="#161616"/><path d="M-9 5.5L-7.3 -2.5L-3.3 0.2L0 -4.3L3.3 0.2L7.3 -2.5L9 5.5H-9Z" fill="#161616"/><rect x="-8.5" y="6.3" width="17" height="2.4" rx="1.2" fill="#161616"/></g></svg></span><span>AFFGOLD</span></a>
            ...
            <div class="footer-grid">
              ...
              <div>
                ...
                <div class="footer-links">
                  <a href="/catalog.html">Все онлайн-казино</a>
...
const sidebar = (links = []) => `<div class="card seo-toc"><h2>На этой странице</h2>${links.map((link) => `<a href="#${link.id}">${escapeHtml(link.title)}</a>`).join('')}</div><div class="card seo-trust"><strong>Как мы работаем</strong><a class="seo-card-link" href="/about/methodology/">Методика рейтинга →</a></div>`;
...
collectionPages.forEach((item) => {
  ...
  writeRoute(item.route, page({
    ...
    content: `${enhancedContent}${projectCards(item.items)}${textPanel('checklist', 'Что проверить самостоятельно', [...item.intro, 'Откройте правила выбранного проекта непосредственно перед действием: условия и доступность методов меняются.'], item.list)}${faqPanel(questions)}`,
    sidebar: sidebar([...sectionLinks, { id: 'checklist', title: 'Чек-лист' }, ...(questions.length ? [{ id: 'faq', title: 'Частые вопросы' }] : [])]),
...
const contactButtons = '<div class="hero-actions contact-actions"><a class="btn btn-primary" href="https://t.me/affgoldbot" target="_blank" rel="noopener noreferrer"><span class="contact-btn-icon" aria-hidden="true">?</span><span>Бот поддержки</span></a></div>';
...
informationalPages.forEach((item) => {
  ...
  writeRoute(item.route, page({
    ...
    content: `${item.blocks.map(([id, title, paragraphs, list]) => textPanel(id, title, paragraphs, list)).join('')}<p>Сведения подготовлены для самостоятельной проверки перед выбором и помогают сопоставить основные условия.</p>${item.route === '/contacts/' ? contactButtons : ''}`,
...
writeRoute(editorialProfile.route, page({
  ...
  content: `${editorialProfile.blocks.map(([id, title, paragraphs, list]) => textPanel(id, title, paragraphs, list)).join('')}`,
